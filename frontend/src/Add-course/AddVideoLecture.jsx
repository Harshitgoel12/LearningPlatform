...
const AddVideoLecture = () => {
  ...
  const selctor= useSelector((state)=>state.Course.course);
    ...
    const handleSubmit= async (e)=>{
       ...
       try {
        ...
        console.log(courseId)
         let result=await axios.post("http://localhost:8080/api/v1/create-section",{section,courseId},{
   ...
   dispatch(addCourseDetails(result.data.updatedcourse))
    ...
    const GotoNextStep= async(req,res)=>{
      nevigate("/dashboard/course/Confirmation")
    }
  return (
    <div className='w-11/12'>
      <form className='w-full bg-[#161D29] rounded-lg' >
        ...
        <button className='bg-yellow-500 px-3  rounded-lg font-bold text-lg self-start py-1.5 text-yellow-black' onClick={GotoNextStep}>Next</button>
        </div>
      </form>
    </div>
  )
